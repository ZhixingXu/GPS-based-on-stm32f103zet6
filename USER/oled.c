 ...
 * @Version: 1.0
 * @Autor: shuai-xv
 * @Date: 2020-05-11 21:29:44
 * @LastEditTime: 2020-05-24 00:12:33
 */ 
#include "oled.h"
#include "char.h"
...
{
	u8 i,k,k1,k2;//y0;
//	if((x1<0)||(x2>128)||(y1<0)||(y2>64)||(x1>x2)||(y1>y2))return;将无符号整数与零进行比较时，编译器将发出此诊断。允许使用整数零进行相等比较，但“> =”和“ <”比较无效
	if(x1==x2)    //画竖线
	{
			for(i=0;i<(y2-y1);i++)
			{
...
/**
 * @brief: configure SPI communication.P
 *          PA5->SCK,PA7->MOSI
 * @param {type} void
 * @retval: 
 */
...
//    GPIO_InitStructure.GPIO_Pin=(uint16_t)0xff1f;//because pin_5,6,7 of port D aren't used
...
//	delayus(5);
//}
//void LcdWriteOrder(uint16_t order){
//    CS(0);
//    RS(0);
//    RD(1);
//    WR(0);
...
//    LcdWriteOrder(0xe2);
//    delayus(1000);
//    //左右方向设置,0xa0正常,0xa1反向
//    LcdWriteOrder(0xa1);
//	delayus(5);
//    //上下方向选择,0xc8反向,0xc0正常方向
//    LcdWriteOrder(0xc8);
//	delayus(5);
//    //0xA6为设置字体为黑色，背景为白色,0xA7为设置字体为白色，背景为黑色
//    LcdWriteOrder(0xa6);
//	delayus(5);
...
//    LcdWriteOrder(0x01);
//	delayus(5);
//    //表格第 18 个命令，这个是个双字节命令，高字节为 0X81，低字节可以选择从 0x00 到 0X3F。用来设置背景光对比度
//    LcdWriteOrder(0x81);
//	delayus(5);
//    LcdWriteOrder(0x23);
//	delayus(5);
...
//    {
//        //--表格第 3 个命令,设置 Y 的坐标,Y 轴有 64 个，一个坐标 8 位,也就是有 8 个坐标,所以一般我们使用的也就是从 0xB0 到 0xB7,就够了
//        LcdWriteOrder(0xb0+i);
//		delayus(5);
//        /*表格第 4 个命令，设置 X 坐标,
//        当你的段初始化为 0xA1 时，X 坐标从 0x10,0x04 到 0x18,0x04,一共 128 位
//        当你的段初始化为 0xA0 时，X 坐标从 0x10,0x00 到 0x18,0x00,一共 128 位
//        在写入数据之后 X 坐标的坐标是会自动加 1 的，我们初始化使用 0xA0 
//        所以我们的 X 坐标从 0x10,0x00 开始*/
//        LcdWriteOrder(0x10);
//		delayus(5);
//        LcdWriteOrder(0x04);
//		delayus(5);
...
//        {
//            LcdWriteData(0xFF);//如果设置背景为白色时，清屏选择 0XFF
//			delayus(5);
//        }
//        
...
//            if(i%2)
//            {
//                LcdWriteData(0xF0);//如果设置背景为白色时，清屏选择 0XFF
//            }
//            else
//            {
//                LcdWriteData(0x0f);
